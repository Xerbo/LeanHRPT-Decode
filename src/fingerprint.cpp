...
class Scoreboard {
   public:
    ...
    void add(T id, size_t weight = 1) { ids[id] += weight; }
    T max() {
        auto largest =
            std::max_element(ids.begin(), ids.end(), [](const auto &p1, const auto &p2) { return p1.second < p2.second; });
